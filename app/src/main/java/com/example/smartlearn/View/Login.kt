...
import com.example.smartlearn.Model.ErrorSession
...
fun LoginPage(modifier: Modifier=Modifier,navController: NavController)
{
    ...
    Column(
        ...
        if(error.value==true)
        {
            Text(text = ErrorSession.error.toString(),modifier = Modifier.padding(0.dp,0.dp,0.dp,20.dp).background(color = Color.Red ,shape = RoundedCornerShape(8.dp)).padding(30.dp,10.dp,20.dp,10.dp), color = Color.White)
            LaunchedEffect(Unit) {
